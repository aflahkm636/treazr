...
const OrderManage = () => {
  ...
  const statusColors = {
    pending: { bg: darkMode ? "bg-yellow-900" : "bg-yellow-50", text: darkMode ? "text-yellow-300" : "text-yellow-800", border: darkMode ? "border-yellow-700" : "border-yellow-200" },
    processing: { bg: darkMode ? "bg-blue-900" : "bg-blue-50", text: darkMode ? "text-blue-300" : "text-blue-800", border: darkMode ? "border-blue-700" : "border-blue-200" },
    shipped: { bg: darkMode ? "bg-purple-900" : "bg-purple-50", text: darkMode ? "text-purple-300" : "text-purple-800", border: darkMode ? "border-purple-700" : "border-purple-200" },
    delivered: { bg: darkMode ? "bg-green-900" : "bg-green-50", text: darkMode ? "text-green-300" : "text-green-800", border: darkMode ? "border-green-700" : "border-green-200" },
    cancelled: { bg: darkMode ? "bg-red-900" : "bg-red-50", text: darkMode ? "text-red-300" : "text-red-800", border: darkMode ? "border-red-700" : "border-red-200" }
  };
  ...
  return (
    <div className={`container mx-auto px-4 py-8 ${darkMode ? "bg-gray-900 text-gray-100" : "bg-gray-50 text-gray-900"}`}>
      ...
      <div className={`p-6 rounded-xl shadow-sm border mb-8 transition-all duration-300 ${darkMode ? "bg-gray-800 border-gray-700 hover:shadow-lg hover:shadow-gray-900/20" : "bg-white border-gray-100 hover:shadow-lg hover:shadow-gray-100"}`}>
      ...
      {/* Order List */}
      <div className={`p-6 rounded-xl shadow-sm border transition-all duration-300 ${darkMode ? "bg-gray-800 border-gray-700 hover:shadow-lg hover:shadow-gray-900/20" : "bg-white border-gray-100 hover:shadow-lg hover:shadow-gray-100"}`}>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
          ...
          <div className="mt-2 sm:mt-0">
            <input
              ...
              className={`px-4 py-2 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-offset-2 ${darkMode ? "bg-gray-700 border-gray-600 text-white focus:ring-blue-500 focus:ring-offset-gray-800" : "bg-white border-gray-300 text-gray-700 focus:ring-blue-500 focus:ring-offset-white"}`}
            />
          </div>
        </div>
                ...
                <tr key={order.id} className={darkMode ? "hover:bg-gray-700/50" : "hover:bg-gray-50"}>
                  ...
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${darkMode ? "text-gray-300" : "text-gray-500"}`}>
                    <button
                      ...
                      className={`flex items-center ${darkMode ? "text-blue-400 hover:text-blue-300" : "text-blue-600 hover:text-blue-800"} hover:underline`}
                    >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${
                      paymentColors[order.paymentMethod?.toLowerCase()]?.bg || paymentColors.other.bg
                    } ${
                      paymentColors[order.paymentMethod?.toLowerCase()]?.text || paymentColors.other.text
                    }`}>
                      {order.paymentMethod || 'Credit'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[order.status]?.bg} ${statusColors[order.status]?.text} ${statusColors[order.status]?.border}`}>
                      {order.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <select
                      ...
                      } ${statusColors[order.status]?.bg} ${darkMode ? "text-white" : "text-gray-900"}`}
                    >
                      <option value="pending" className={darkMode ? "bg-gray-800" : "bg-white"}>
                        Pending
                      </option>
                      <option value="processing" className={darkMode ? "bg-gray-800" : "bg-white"}>
                        Processing
                      </option>
      ...
      {selectedOrderItems && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 transition-opacity">
          <div className={`rounded-xl max-w-md w-full max-h-[80vh] overflow-y-auto shadow-xl transform transition-all ${darkMode ? "bg-gray-800" : "bg-white"}`}>
            <div className={`flex justify-between items-center border-b p-4 sticky top-0 z-10 ${darkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
              <h3 className={`text-lg font-semibold ${darkMode ? "text-white" : "text-gray-800"}`}>Order Items</h3>
              <button
                onClick={() => setSelectedOrderItems(null)}
                className={`p-1 rounded-full hover:bg-opacity-20 transition-colors ${darkMode ? "text-gray-400 hover:text-gray-300 hover:bg-gray-700" : "text-gray-400 hover:text-gray-500 hover:bg-gray-100"}`}
              >
              ...
              <ul className={`divide-y ${darkMode ? "divide-gray-700" : "divide-gray-200"}`}>
                {selectedOrderItems.map((item, index) => (
                  <li key={index} className="py-4">
                    <div className="flex">
                      <div className={`flex-shrink-0 h-16 w-16 rounded-lg overflow-hidden ${darkMode ? "bg-gray-700" : "bg-gray-100"}`}>
                        {item.image ? (
                          <img
                            src={item.image}
                            alt={item.name}
                          ...
                          <div className="w-full h-full flex items-center justify-center">
                            <FiPackage className={`w-6 h-6 ${darkMode ? "text-gray-500" : "text-gray-400"}`} />
                          </div>
                        )}
                      </div>
                      <div className="ml-4 flex-1">
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-2 text-sm">
                          ...
                          <div className={`col-span-2 ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
            ...
            <div className={`border-t p-4 sticky bottom-0 ${darkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-white"}`}>
              <button
                onClick={() => setSelectedOrderItems(null)}
